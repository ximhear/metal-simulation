...
public struct EquilibriumTables {
    ...
    public let bulgeRadii: [Float]
    public let bulgeSpeeds: [Float]
    public let diskKinematics: [SIMD4<Float>]

    public init(diskMass md: Double = GalaxyPhysics.diskMass,
                bulgeMass mb: Double = 0,
                bulgeScale ab: Double = 0.25,
                bulgeCutoff cb: Double = 2,
                ...
                speedMaxRadius: Double = EquilibriumTables.maxRadius,
        ...
        func profile(mass target: Double, scale: Double, cutoff: Double) -> (rho: [Double], mass: [Double]) {
            var rho = radii.map { r in scale / (2 * Double.pi * r * pow(r + scale, 3)) * exp(-pow(r / cutoff, 2)) }
            ...
            let normalization = target / mass[n - 1]
            rho = rho.map { $0 * normalization }; mass = mass.map { $0 * normalization }
            return (rho, mass)
        }
        let halo = profile(mass: mh, scale: a, cutoff: cutoff)
        let bulge = profile(mass: mb, scale: ab, cutoff: cb)
        let mass = zip(halo.mass, bulge.mass).map(+)
        let totalMass = zip(radii, mass).map { r, m in m + md * (1 - (1 + r) * exp(-r)) }
        var psi = [Double](repeating: 0, count: n)
        psi[n - 1] = (mh + mb + md) / radii[n - 1]
        ...
        func sample(rho: [Double], tracerMass: [Double], scale: Double, cutoff: Double) -> ([Float], [Float]) {
            var second = [Double](repeating: 0, count: n)
            for i in 0..<n {
                let r = radii[i]
                let slope = -1 / r - 3 / (r + scale) - 2 * r / (cutoff * cutoff)
                let rhoPrime = rho[i] * slope
                let rhoSecond = rho[i] * (slope * slope + 1 / (r * r) + 3 / pow(r + scale, 2) - 2 / (cutoff * cutoff))
                let p1 = -totalMass[i] / (r * r)
                let massPrime = 4 * .pi * r * r * (halo.rho[i] + bulge.rho[i]) + md * r * exp(-r)
            ...
            let sampledRadii = (0..<Self.radialCount).map {
                Float(interpolate((Double($0) + 0.5) / Double(Self.radialCount) * tracerMass.last!, xs: tracerMass, ys: radii))
            }
            var speeds = [Float]()
            for row in 0..<Self.radiusBins {
                let r = Self.minRadius * pow(speedMaxRadius / Self.minRadius, Double(row) / Double(Self.radiusBins - 1))
            ...
            return (sampledRadii, speeds)
        }
        (haloRadii, haloSpeeds) = sample(rho: halo.rho, tracerMass: halo.mass, scale: a, cutoff: cutoff)
        if mb > 0 {
            (bulgeRadii, bulgeSpeeds) = sample(rho: bulge.rho, tracerMass: bulge.mass, scale: ab, cutoff: cb)
        } else {
            bulgeRadii = haloRadii; bulgeSpeeds = haloSpeeds
        }
